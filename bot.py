...
Вам скучно? Ищете как бы себя развлечь, с кем подискутировать на серъёзные темы или, может быть, хотите просто пообщаться с такими же людьми?
...
def addChat(update, context):
    update.message.reply_text('Пока мы автоматизируем данную функцию, вы можете написать @username1 или @username2 чтобы узнать как добавить свой чат.')
