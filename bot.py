...
from telegram import ParseMode, InlineKeyboardButton, InlineKeyboardMarkup, InlineQueryResultArticle, InlineQueryResultGif, InputTextMessageContent
...
def checkquery(update, context):
    ...
    if ids in str(members):
        ...
        cursor.execute('SELECT id FROM pussy WHERE type=gif ORDER BY random() LIMIT 1')
        pussy = cursor.fetchall()
        pussies = pussy[0]
        try:
